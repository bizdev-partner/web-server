...
export class ActivityService implements IActivityService {
    ...
    async getActivityDetails(query: Contracts.GetActivityDetailsQuery): Promise<Activity> {
        const activity = await this.activities.findById(query.activityId);
        if (!activity) throw new Error('Activity not found');
        return activity;
    }

    async scheduleActivity(command: Contracts.ScheduleActivityCommand): Promise<Activity> {
    ...
    async completeActivity(command: Contracts.CompleteActivityCommand): Promise<Activity> {
    ...
    async cancelActivity(command: Contracts.CancelActivityCommand): Promise<Activity> {
        const activity = await this.getActivityDetails(new Contracts.GetActivityDetailsQuery(command.activityId));
        activity.cancel();
        await this.activities.update(activity);
        return activity;
    ...
    async rescheduleActivity(command: Contracts.RescheduleActivityCommand): Promise<Activity> {
        const activity = await this.getActivityDetails(new Contracts.GetActivityDetailsQuery(command.activityId));
